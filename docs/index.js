...
// node_modules/@devcapsule/adapter/dist/esm/style.js
function _addStyle(styleNode, css4, parentNode = document.head) {
  styleNode.textContent = css4;
  parentNode.append(styleNode);
}
__name(_addStyle, "_addStyle");
function addStyle(css4) {
  const styleNode = document.createElement("style");
  _addStyle(styleNode, css4);
  return styleNode;
}
__name(addStyle, "addStyle");

// node_modules/@devcapsule/adapter/dist/esm/adapter.js
var DOMError = class extends Error {
  static {
    __name(this, "DOMError");
  }
  constructor(message) {
    super();
...
function AdapterMixin(Base) {
  ...
  return _a = class extends Base {
    ...
    static addStyle(css4) {
      const styleNode = document.createElement("style");
      styleNode.setAttribute("component", this.name);
      _addStyle(styleNode, css4, document.head);
    }
    static define(tagName) {
      try {
        customElements.define(tagName, this);
        ...
        if (error instanceof DOMException) {
          throw new DOMError(`DOMException: '${this.name}' has already been defined to tag '${this.tagName}'`);
        }
      }
      this.tagName = tagName;
      this.defineStyle();
...
${selector} { ${style.css} }`;
    ...
    static tagStyle(css4) {
      if (this.tagName) {
        this.addStyle(`
${this.tagName} { ${css4} }`);
      }
      this.styles = this.styles.concat({ class_: "", css: css4 });
    }
    static classStyle(class_, css4) {
      if (this.tagName) {
        this.addStyle(`
${this.tagName}.${class_} { ${css4} }`);
      }
      this.styles = this.styles.concat({ class_, css: css4 });
    }
    static _generate_id() {
      if (this.idsCount > 1e4) {
  ...
  ], _a._is_styled = false, _a.max_id = Math.pow(16, 4) - 1, _a.ids = {}, _a.idsCount = 0, _a;
}
__name(AdapterMixin, "AdapterMixin");
var Adapter2 = AdapterMixin(HTMLElement);
